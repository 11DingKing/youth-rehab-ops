...
func (s *Store) RecordFollowUp(ctx context.Context, actor domain.Actor, follow domain.FollowUp) (domain.FollowUp, error) {
	...
	err := withTx(ctx, s.db, func(tx *sql.Tx) error {
		result, err := tx.ExecContext(ctx, `INSERT INTO followups(plan_id,plan_version,professional_id,pain_score,mobility_score,load_tolerance,notes,assessed_at,valid_until,created_at)
		...
		if err != nil {
			return fmt.Errorf("insert follow-up: %w", err)
		}
		if err := requireAffected(result, "follow-up"); err != nil {
			return domain.ErrForbidden
		}
		follow.ID, _ = result.LastInsertId()
		return appendAudit(ctx, tx, audit.Record{ActorID: actor.UserID, ActorRole: string(actor.Role), Action: "followup.recorded",
			ObjectType: "followup", ObjectID: fmt.Sprint(follow.ID), Result: audit.Succeeded, RequestID: actor.RequestID, CreatedAt: follow.CreatedAt})
	})
	if err != nil {
		return domain.FollowUp{}, err
	}
