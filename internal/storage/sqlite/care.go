...
func (s *Store) PublishPlanVersion(ctx context.Context, actor domain.Actor, planID, expected int64, version domain.RehabPlanVersion) (domain.RehabPlanVersion, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT id,referral_id,professional_id,current_version,active,created_at,updated_at FROM rehab_plans WHERE id=?`, planID).
		Scan(&plan.ID, &plan.ReferralID, &plan.ProfessionalID, &plan.CurrentVersion, &active, &created, &updated)
	if err == sql.ErrNoRows {
		return domain.RehabPlanVersion{}, domain.ErrNotFound
	}
	if err != nil {
		return domain.RehabPlanVersion{}, fmt.Errorf("read rehab plan: %w", err)
	}
	...
	if err := version.Validate(now); err != nil {
		return domain.RehabPlanVersion{}, err
	}
	publication, err := plan.PreparePublication(expected, now)
	if err != nil {
		return domain.RehabPlanVersion{}, err
	}
	err = withTx(ctx, s.db, func(tx *sql.Tx) error {
		result, err := tx.ExecContext(ctx, `UPDATE rehab_plans SET current_version=?,updated_at=? WHERE id=? AND current_version=? AND active=1`,
			publication.NextVersion, timeText(now), planID, publication.Expected)
		if err != nil {
	...
	version.Version = publication.NextVersion
