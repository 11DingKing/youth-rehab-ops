...
func (s *Store) GrantClearance(ctx context.Context, actor domain.Actor, clearance domain.Clearance) (domain.Clearance, error) {
	return s.grantClearance(ctx, actor, clearance, true)
}

func (s *Store) CreateClearanceRecord(ctx context.Context, actor domain.Actor, clearance domain.Clearance) (domain.Clearance, error) {
	return s.grantClearance(ctx, actor, clearance, false)
}

func (s *Store) grantClearance(ctx context.Context, actor domain.Actor, clearance domain.Clearance, releaseBlock bool) (domain.Clearance, error) {
	err := withTx(ctx, s.db, func(tx *sql.Tx) error {
		var follow domain.FollowUp
		var assessed, validUntil, created string
		err := tx.QueryRowContext(ctx, `SELECT id,plan_id,plan_version,professional_id,pain_score,mobility_score,load_tolerance,notes,assessed_at,valid_until,created_at FROM followups WHERE id=?`, clearance.FollowUpID).
		...
		result, err := tx.ExecContext(ctx, `INSERT INTO clearances(incident_id,followup_id,professional_id,kind,conditions,status,valid_from,valid_until,version,created_at,updated_at)
		...
		if releaseBlock {
			if _, err := tx.ExecContext(ctx, `UPDATE training_blocks SET active=0,released_at=? WHERE incident_id=? AND active=1`,
				timeText(clearance.UpdatedAt), clearance.IncidentID); err != nil {
				return fmt.Errorf("release training block after clearance: %w", err)
			}
		}
		return appendAudit(ctx, tx, audit.Record{ActorID: actor.UserID, ActorRole: string(actor.Role), Action: "clearance.granted",
...
func (s *Store) ReleaseClearanceTrainingBlock(ctx context.Context, actor domain.Actor, incidentID int64, now time.Time) error {
	return withTx(ctx, s.db, func(tx *sql.Tx) error {
		...
		return requireAffected(result, "training block")
	})
}
