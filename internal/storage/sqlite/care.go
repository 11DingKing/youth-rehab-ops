...
func (s *Store) StartPlanVersionPublication(ctx context.Context, actor domain.Actor, planID, expected int64, version domain.RehabPlanVersion) <-chan repository.PlanPublicationResult {
	result := make(chan repository.PlanPublicationResult, 1)
	go func() {
		published, err := s.PublishPlanVersion(ctx, actor, planID, expected, version)
		result <- repository.PlanPublicationResult{Version: published, Err: err}
	}()
	return result
}
