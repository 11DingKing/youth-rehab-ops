...
func (s *CareService) PublishPlan(ctx context.Context, actor domain.Actor, planID, expected int64, goals, restrictions, exercises string, due time.Time) (domain.RehabPlanVersion, error) {
	...
	result := s.repo.StartPlanVersionPublication(context.WithoutCancel(ctx), actor, planID, expected, version)
	select {
	case <-ctx.Done():
		return domain.RehabPlanVersion{}, ctx.Err()
	case published := <-result:
		return published.Version, published.Err
	}
}
