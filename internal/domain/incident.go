...
func (i Incident) PrepareCorrection(reason string, correctedBy, revision int64, now time.Time) (CorrectionPlan, error) {
	if revision <= 0 || strings.TrimSpace(reason) == "" {
		return CorrectionPlan{}, &FieldError{Field: "correction", Problem: "revision and reason are required"}
	}
	return CorrectionPlan{Expected: i.Version, Revision: IncidentRevision{IncidentID: i.ID, Revision: revision,
		BodyArea: i.BodyArea, OccurredAt: i.OccurredAt, Description: i.Description, Reason: strings.TrimSpace(reason),
		CorrectedBy: correctedBy, CreatedAt: now}}, nil
}
