...
func (p RehabPlan) PreparePublication(expected int64, now time.Time) (PlanPublication, error) {
	if !p.Active {
		return PlanPublication{}, fmt.Errorf("inactive rehabilitation plan: %w", ErrConflict)
	}
	if p.CurrentVersion != expected {
		return PlanPublication{}, &ConflictError{Entity: "rehab_plan", Expected: expected, Actual: p.CurrentVersion}
	}
	return PlanPublication{PlanID: p.ID, Expected: expected, NextVersion: expected + 1, PublishedAt: now}, nil
}
